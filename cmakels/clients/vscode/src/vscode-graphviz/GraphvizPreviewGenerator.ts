...
export class GraphvizPreviewGenerator extends Disposable {
    ...
    webviewPanels = new Map<string, PreviewPanel>();
    ...
    async revealOrCreatePreview(id: string, doc: string, displayColumn: ViewColumn): Promise<void> {
    ...
    toSvg(doc: string): Thenable<string> | string {
        return new Viz({Module, render}).renderString(doc);
    }

    private async getPreviewHtml(previewPanel: PreviewPanel, doc: string): Promise<string> {
...
class PreviewPanel {
    ...
    constructor(public dotString: string, private panel: WebviewPanel) {}
